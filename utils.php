...
define('VIKPAYSTACK_DIR', dirname(__FILE__));
// Define plugin base URI
define('VIKPAYSTACK_URI', plugin_dir_url(__FILE__));
 ...
 */
function vikpaystack_load_payment($plugin)
{
    if (!JLoader::import("{$plugin}.paystack", VIKPAYSTACK_DIR))
    {
        // there is not a version available for the given plugin
        return false;
    }

    return ucwords($plugin) . 'PaystackPayment';
}
 ...
 */
function vikpaystack_get_payment_path($plugin)
{
    $path = VIKPAYSTACK_DIR . DIRECTORY_SEPARATOR . $plugin . DIRECTORY_SEPARATOR . 'paystack.php';
